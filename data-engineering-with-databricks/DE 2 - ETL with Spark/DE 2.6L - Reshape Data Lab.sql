...
SELECT *
FROM (select user_id as user, event_name, count(*) as event_count
from events 
group by user_id, event_name) sub
PIVOT (
  sum(sub.event_count) for event_name IN (
...
-- MAGIC
-- MAGIC event_countDF = (spark
-- MAGIC         .table('events')
-- MAGIC         .select('user_id', 'event_name')
-- MAGIC         .groupby('user_id', 'event_name')
-- MAGIC         .agg(count('event_name').alias('event_count'))
-- MAGIC     )
-- MAGIC
-- MAGIC events_pivotDF = (event_countDF
-- MAGIC         .withColumnRenamed('user_id', 'user')
-- MAGIC         .groupby('user')
-- MAGIC         .pivot('event_name')
-- MAGIC         .sum('event_count'))
-- MAGIC             
-- MAGIC events_pivotDF.createOrReplaceTempView("events_pivot")
-- MAGIC
-- MAGIC display(events_pivotDF)
...
-- MAGIC
-- MAGIC clickpathsDF = (events_pivotDF
-- MAGIC                 .join(transactionsDF, events_pivotDF.user == transactionsDF.user_id)
-- MAGIC                 )
-- MAGIC
-- MAGIC display(clickpathsDF)
-- MAGIC
-- MAGIC clickpathsDF.createOrReplaceTempView("clickpaths")
